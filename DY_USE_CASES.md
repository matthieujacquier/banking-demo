...
5. **Events to expect in reports** — standard: pageviews, `identify-v1`, `login-v1`, `signup-v1`, `keyword-search-v1`, `add-to-cart-v1` (Application Started), `purchase-v1` (Submission / Purchase), `inform-affinity-v1` (`categories`, `goals`, `personas`). Custom: `LoanQuoteRequested`, `Time on Category`, `Financial Profile Updated`, `Search Result Click`, `Home Recommendation Click`, `Offer Click`, `Advisor Contact Request`, `Shopping Muse Chat Open / Message Sent / Product Click`, `Search Muse Handoff Shown / Clicked`, `Add to Compare` / `Remove from Compare` / `Compare Opened`. Engagements: `IMP`/`CLICK` on decisions, `SLOT_CLICK` on search, Muse and recommendation slots.
...
- **Comparison shoppers** — `Add to Compare` in the session → hero 1.1 and Offers 5.1 with the compared category; `Compare Opened` with `compareCount` ≥ 2 is the strongest pre-application intent the site emits. Property filters make it sharper: `category`, `riskLevel`, `aer` (e.g., anyone comparing savings above 3% AER), `surface` (`pdp` vs `listing`).
